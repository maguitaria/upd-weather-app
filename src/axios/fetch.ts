...
export async function get7DaysForecast(latitude, longitude) {
  ...
  const dailyInfo = weatherData.daily.time.map((day, index) => {
    return {
      ...
      description:
        weatherCode_JSON[weatherData.daily.weather_code[index]]?.day
          ?.description || "", // Adjust based on the actual structure of your API response
      icon:
        weatherCode_JSON[weatherData.daily.weather_code[index]]?.day?.image ||
